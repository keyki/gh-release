...
release-create() {
	...
	upload_url="$(curl -s -u "$GITHUB_USERNAME:$GITHUB_ACCESS_TOKEN" -d "$release" "$release_url" | upload-url)"
	for asset in $(ls -A release); do
		local name="$(basename $asset)"
		echo "Uploading $name ..."
		curl -X POST -u "$GITHUB_USERNAME:$GITHUB_ACCESS_TOKEN" -H "Content-Type: $(mimetype $name)" --data-binary "@release/$asset" \
...
release-destroy() {
	...
	release_id="$(curl -s -u "$GITHUB_USERNAME:$GITHUB_ACCESS_TOKEN" "$release_url" | release-id-from-tagname "$version")"
	echo "Deleting release..."
	curl -s -u "$GITHUB_USERNAME:$GITHUB_ACCESS_TOKEN" -X DELETE "$release_url/$release_id"
	echo "Deleting tag..."
	tag_url="$(printf "$ref_endpoint" "$reponame" "$version")"
	curl -s -u "$GITHUB_USERNAME:$GITHUB_ACCESS_TOKEN" -X DELETE "$tag_url"
}
